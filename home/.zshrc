...
# TERM guard: non-interactive shells (scripted ssh, CI) arrive with no $TERM,
# and plugins that probe colors via tput at load (e.g. zsh-you-should-use)
# then spam "tput: No value for $TERM" to stderr, and starship errors under
# TERM=dumb — so default to a real terminfo entry. Interactive terminals always
# set TERM, so this only ever fills the empty case.
: "${TERM:=xterm-256color}"
export TERM
